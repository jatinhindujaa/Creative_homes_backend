...
//   const { position, type, location, jd, status } = req.body;
...
const createOpening = asyncHandler(async (req, res) => {
  console.log(req.body); // Log the request body to see if all fields are included
  const { position, type, location, jd, status } = req.body;

  // Validation check for required fields
  if (!position || !type || !location || !jd || !status) {
    throw new ApiError(400, "Please fill the required fields!!!");
  }
